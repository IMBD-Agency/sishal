...
@include('erp.components.sidebar')
    ...
    @include('erp.components.header')
        ...
        <div class="container-fluid px-4 py-3 bg-white border-bottom">
            <div class="row align-items-center">
                <div class="col-md-8">
                    ...
                    <h2 class="fw-bold mb-0">Create Coupon</h2>
                    <p class="text-muted mb-0">Add a new promo code or discount coupon.</p>
                </div>
        ...
        <div class="container-fluid px-4 py-4">
            ...
            <form action="{{ route('coupons.store') }}" method="POST" id="couponForm" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                        <!-- Basic Information -->
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" min="1" class="form-control @error('user_limit') is-invalid @enderror" 
                                                   id="user_limit" name="user_limit" value="{{ old('user_limit', 1) }}" required>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('scope_type') is-invalid @enderror" id="scope_type" name="scope_type" required>
                                        ...
                                        <option value="categories" {{ old('scope_type') == 'categories' ? 'selected' : '' }}>Specific Categories</option>
                                        <option value="products" {{ old('scope_type') == 'products' ? 'selected' : '' }}>Specific Products</option>
                                        <option value="exclude_categories" {{ old('scope_type') == 'exclude_categories' ? 'selected' : '' }}>Exclude Categories</option>
                                        <option value="exclude_products" {{ old('scope_type') == 'exclude_products' ? 'selected' : '' }}>Exclude Products</option>
                                    </select>
                                    @error('scope_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                        ...
                                        @foreach($categories as $index => $category)
                                            <div class="form-check mb-2 excluded-category-item {{ $index >= 30 ? 'd-none' : '' }}" data-name="{{ strtolower($category->name) }}" data-index="{{ $index }}">
                                                <input class="form-check-input" type="checkbox" 
                                                       ...
                                                       {{ in_array($category->id, old('excluded_categories', [])) ? 'checked' : '' }}>
                                        ...
                                        @foreach($products as $index => $product)
                                            <div class="form-check mb-2 excluded-product-item {{ $index >= 30 ? 'd-none' : '' }}" data-name="{{ strtolower($product->name . ' ' . ($product->sku ?? '')) }}" data-index="{{ $index }}">
                                                <input class="form-check-input" type="checkbox" 
                                                       ...
                                                       {{ in_array($product->id, old('excluded_products', [])) ? 'checked' : '' }}>
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="free_delivery" name="free_delivery" value="1"
                                           {{ old('free_delivery', false) ? 'checked' : '' }}>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('Coupon form script loaded');
            console.log('Form element:', couponForm);
        ...
        if (document.readyState === 'loading') {
            ...
            console.log('DOM already loaded, initializing coupon form');
            const couponForm = document.getElementById('couponForm');
            const submitBtn = document.getElementById('submitBtn');
            if (couponForm && submitBtn) {
                submitBtn.addEventListener('click', function(e) {
