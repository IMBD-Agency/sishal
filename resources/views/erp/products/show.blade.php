        ...
        <div class="container-fluid px-4 py-3 bg-white border-bottom">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('product.list') }}"
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <div class="d-flex flex-wrap align-items-center gap-3">
                                <span
                                    class="badge bg-success fs-6 px-3 py-2">{{ $product->status == 'active' ? 'Active' : 'Inactive' }}</span>
                                @if($product->brand)
                                    <span class="badge bg-primary fs-6 px-3 py-2"><i
                                            class="fas fa-tag me-1"></i>{{ $product->brand->name }}</span>
                                @endif
                                ...
                                <ul class="dropdown-menu">
                                    @can('edit products')
                                        <li><a class="dropdown-item" href="{{ route('product.edit', $product->id) }}"><i
                                                    class="fas fa-edit me-2"></i>Edit Product</a></li>
                                    @endcan
                                    <li>
                                        ...
                                        <div class="col-sm-6">
                                            <div class="border-start border-success border-4 ps-3">
                                                <h6 class="text-success mb-1">
                                                    {{ (($product->discount && $product->discount > 0 ? $product->discount : $product->price) - ($product->cost ?? 0)) }}৳
                                                </h6>
                                    ...
                                    <div class="col-lg-3 col-md-4 col-sm-6">
                                        <div class="position-relative">
                                            ...
                                            <div class="position-absolute top-0 end-0 m-2">
                                                <form action="{{ route('product.gallery.delete', $gallery->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        <div class="col-6">
                            <div class="card border-0 bg-success text-white">
                                <div class="card-body text-center py-4">
                                    ...
                                    <h4 class="fw-bold mb-1">{{ $unitsSold }}</h4>
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            @forelse($branchStocks as $stock)
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="fw-medium">{{ $stock['branch_name'] }}</span>
                                        <span class="badge bg-primary">{{ $stock['quantity'] }} units</span>
                                    </div>
                                    <div class="progress" style="height: 8px;">
                                        <div class="progress-bar bg-primary"
                                            style="width: {{ $totalStock > 0 ? ($stock['quantity'] / $totalStock) * 100 : 0 }}%">
                                        </div>
                                    </div>
                                    <small class="text-muted">Updated:
                                        {{ $stock['updated_at'] ? \Carbon\Carbon::parse($stock['updated_at'])->format('Y-m-d') : 'N/A' }}</small>
                                </div>
                            @empty
                                <div class="text-center text-muted py-3">
                                    <i class="fas fa-boxes fa-2x mb-2"></i>
                                    <p class="mb-0">No branch stock available</p>
                                </div>
                            @endforelse
                            ...
                            @forelse($warehouseStocks as $stock)
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="fw-medium">{{ $stock['warehouse_name'] }}</span>
                            ...
                            @empty
                                @if($branchStocks->isEmpty())
                                    <div class="text-center text-muted py-3">
                                        ...
                                        <p class="mb-0">No warehouse stock available</p>
                                    </div>
                                @endif
                            @endforelse
                        </div>
                            ...
                            @forelse($recentActivity as $activity)
                                <div class="d-flex align-items-start mb-3">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <small class="text-muted">{{ $activity['details'] }} • {{ $activity['time'] }}</small>
                                    </div>
                                </div>
                            @empty
                                <div class="text-center text-muted py-3">
