        ...
        <table class="table mb-0" id="adjustmentTable">
            ...
            <tbody>
                ...
                <tr id="adj-row-{{ $item->id }}">
                    ...
                    <td class="text-nowrap">{{ $item->adjustment?->date ? $item->adjustment->date->format('d-M-Y') :
                        'N/A' }}</td>
                    <td>
                        <div class="d-flex align-items-center">
                            <div>
                                <div class="fw-bold text-dark">{{ $item->product->name ?? 'Deleted Product' }}</div>
                                <div class="small text-muted">{{ $item->product->category->name ?? 'Uncategorized' }}
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td>
                        ...
                        <span class="badge bg-light text-dark border"><i class="fas fa-store me-1"></i>{{
                            $item->adjustment->branch->name }}</span>
                        @elseif($item->adjustment?->warehouse)
                        <span class="badge bg-light text-primary border"><i class="fas fa-warehouse me-1"></i>{{
                            $item->adjustment->warehouse->name }}</span>
                        @else
                    ...
                    <td class="text-center">
                        ...
                        @elseif($diff < 0) <span class="text-danger fw-bold">{{ $diff }}</span>
                            @else
                    ...
                    <td class="text-center">
                        @if($item->adjustment)
                        @can('delete stock')
                        <button type="button" class="btn btn-sm btn-outline-danger btn-delete-adjustment"
                            data-adj-id="{{ $item->adjustment->id }}"
                            data-adj-number="{{ $item->adjustment->adjustment_number }}"
                            data-product="{{ $item->product->name ?? 'this product' }}" data-diff="{{ $diff }}"
                ...
                @empty
                <tr>
                    <td colspan="11" class="text-center py-5 text-muted">No adjustments found matching your filters.
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="card-footer bg-white border-top p-3">
        <div class="d-flex justify-content-between align-items-center">
            <div class="small text-muted">
                Showing {{ $adjustments->firstItem() }} to {{ $adjustments->lastItem() }} of {{ $adjustments->total() }}
                entries
            </div>
        ...
        <div class="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
            ...
            <div class="modal-body p-4">
                <div class="alert alert-warning border-0 rounded-2 mb-3" style="background:#fff8e1;">
                    <div class="d-flex align-items-start gap-2">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Deleting this adjustment will undo its stock change and restore
                                the previous quantity.</small>
                        </div>
                    </div>
                </div>
                <div class="bg-light rounded-2 p-3 mb-1">
                    <div class="row g-2">
                        <div class="col-6">
                            <div class="small text-muted text-uppercase fw-bold" style="font-size:0.7rem;">Adjustment No
                            </div>
                        ...
                        <div class="col-6 mt-2">
                            <div class="small text-muted text-uppercase fw-bold" style="font-size:0.7rem;">Qty Change
                            </div>
                            <div class="fw-bold" id="modal-adj-diff">—</div>
                        </div>
                        <div class="col-6 mt-2">
                            <div class="small text-muted text-uppercase fw-bold" style="font-size:0.7rem;">After
                                Reversal</div>
