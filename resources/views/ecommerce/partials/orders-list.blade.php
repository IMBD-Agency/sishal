                        ...
                        <div class="flex-grow-1">
                            @if($item->product)
                                <a href="{{ route('product.details', $item->product->slug) }}"
                                    class="d-block fw-semibold text-dark text-decoration-none small">{{ $item->product->name }}</a>
                            @else
        ...
        <div class="d-flex justify-content-between align-items-center pt-2 border-top mt-2">
            <div class="d-flex gap-2">
                ...
                @if(($order->status === 'pending' || $order->status === 'approved') && !($order->invoice && $order->invoice->status === 'paid'))
                    <button type="button" class="btn-outline-simple btn-sm btn-cancel-order" data-bs-toggle="modal"
                        data-bs-target="#cancelOrderModal">
                        Cancel Order
                    </button>
                @endif
                @if($order->status === 'cancelled')
                    <button type="button" class="btn-outline-simple btn-sm btn-delete-order" data-bs-toggle="modal"
                        data-bs-target="#deleteOrderModal" data-order-number="{{ $order->order_number }}">
                        Delete Order
                    </button>
                @endif
            </div>
